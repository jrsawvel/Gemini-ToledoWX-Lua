...
if global_dt.yday > 67 and global_dt.yday < 306 then
    B_ISDST = true    
    S_TZ = "edt"
end
...
local function create_mesoscale_file(h)
    ...
    local img = '=> ' ..  config.get_value_for("imagehome") .. '/' .. gif_file .. '    ' .. gif_file
    ...
    local gmi_output = page.get_output("Mesoscale Discussion")
...
local function process_md_item(h_item)
    ...
    if regional_md(mddesc) then
        ...
        mdhash.mdfilename = gmidir .. "mesoscale" .. mdnum .. ".gmi"
        mdhash.mdnum = mdnum
        mdhash.mdtim = mdtime
        ...
        if not io.open(mdhash.mdfilename, "r") then
            email.send_alert("Mesoscale Discussion " .. mdnum, config.get_value_for("wxhome") .. "/" .. "mesoscale" .. mdnum .. ".gmi")
        end
...
print("DEBUG TYPE EVENT: " .. type(event) .. "  strlen = " .. string.len(event))
     ...
     if does_alert_exist(a_zone_alerts, event) == false and string.len(event) > 0 then
        ...
        filename = "lc-alert-" .. filename .. ".gmi"

        page.set_template_name("specialstatement")
        page.set_template_variable("msg", msg)
        page.set_template_variable("basic_page", true)
        local gmi_output = page.get_output(event)

        local output_filename =  config.get_value_for("gmidir") .. filename
        local o = assert(io.open(output_filename, "w"))
        o:write(gmi_output)
        o:close()
...
--    msg = rex.match(x_text, "<h3>" .. k .. "</h3><pre>(.*)</pre><hr /><br /><h3>", 1, "s")
    ...
    page.set_template_variable("msg", msg)
    page.set_template_variable("basic_page", true)
    local gmi_output = page.get_output(k)
...
local gmi_output = page.get_output("Toledo Weather")

local output_filename =  config.get_value_for("gmidir") .. config.get_value_for("wx_index_output_file")
local o = assert(io.open(output_filename, "w"))
o:write(gmi_output)
o:close()


local hwo_file = config.get_value_for("gmidir") .. config.get_value_for("hwo_text_file")
local hwo_f
...
for ctr=1, #lc_alerts do
    if lc_alerts[ctr].alert ~= "Hazardous Weather Outlook" then
        ...
        local alert_exists_filename = config.get_value_for("gmidir") .. utils.clean_title(tmp_str) .. os.date("%d%b%Y",   os.time()) .. ".txt"
