...
for i=1, #minutely do
    ...
    if m.precipIntensity > 0.0 then
        ...
        hash.precipIntensity = dsutils.calc_precip_intensity_and_color(m.precipIntensity)
...
local precip_desc = dsutils.calc_precip_intensity_and_color(currently.precipIntensity)
...
for i=1, 24 do
    ...
    if h.precipIntensity > 0.0 then
        ...
        local x = dsutils.calc_precip_intensity_and_color(h.precipIntensity)
...
local gmi_output = page.get_output("Dark Sky Data")

local output_filename =  config.get_value_for("gmidir") .. config.get_value_for("wx_darksky_output_file")
